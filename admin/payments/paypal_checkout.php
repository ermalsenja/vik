...
/**
 * This class is used to collect payments through VikRentCar
 * by using the PayPal Checkout integration (JavaScript SDK).
 *
 * @since 	1.15.5 (J) - 1.4.0 (WP)
 */
class VikRentCarPayPalCheckoutPayment extends JPayment
{	
	/**
	 * @override
	 * Class constructor.
	...
	protected function beginTransaction()
	{
		...
		if (strlen($tn_item_name) >= 127) {
			$tn_item_name = $multib_substr ? mb_substr($tn_item_name, 0, 127) : substr($tn_item_name, 0, 127);
		}
	...
	protected function validateTransaction(JPaymentStatus &$status)
	{
		...
		$totPaid = 0;
		$paypal_currency = VikRentCar::getCurrencyCodePp();
