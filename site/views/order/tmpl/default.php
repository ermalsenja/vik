...
<div class="warn">
	...
	<span><?php echo JText::_('VRC_YOURORDER_PENDING'); ?></span>
</div>
	<?php
} else {
	// cancelled
...
<div class="err">
	...
	<span><?php echo JText::_('VRC_YOURORDER_CANCELLED'); ?></span>
</div>
	<?php
}
?>
	...
	<div class="vrc-order-details-top-order">
		...
		<div class="vrc-order-details-top-element vrc-order-details-top-cdet">
			<div class="vrc-order-details-text-wrap">
				<span class="vrcvordudatatitle"><?php echo JText::_('VRPERSDETS'); ?></span> <?php echo nl2br($ord['custdata']); ?>
			<?php
			...
			if (VikRentCar::allowDocsUpload() && $ord['status'] == 'confirmed' && count($customer) && mktime(23, 59, 59, $info_from['mon'], $info_from['mday'], $info_from['year']) >= time()) {
				...
				<div class="vrc-order-details-info-inner">
					<span class="vrc-order-details-info-val vrc-order-details-info-val-upload-docs">
						<a href="<?php echo JRoute::_('index.php?option=com_vikrentcar&view=docsupload&sid='.$ord['sid'].'&ts='.$ord['ts'].(!empty($bestitemid) ? '&Itemid='.$bestitemid : (!empty($pitemid) ? '&Itemid='.$pitemid : ''))); ?>" class="btn vrc-pref-color-btn"><i class="<?php echo VikRentCarIcons::i(($has_uploaded_docs ? 'check-circle' : 'exclamation-circle')); ?>"></i> <?php echo JText::_('VRC_UPLOAD_DOCUMENTS'); ?></a>
					</span>
				</div>
				<?php
			}
		...
		<div class="vrc-order-details-top-element vrc-order-details-top-odet">
			<div class="vrc-order-details-text-wrap">
				<span class="vrcvordudatatitle"><?php echo JText::_('VRCORDERDETAILS'); ?></span>
				<div class="vrc-order-details-info-inner">
					<span class="vrc-order-details-info-key"><?php echo JText::_('VRORDEREDON'); ?></span> 
			...
			if ($ord['status'] == 'confirmed') {
				?>
				<div class="vrc-order-details-info-inner">
					<span class="vrc-order-details-info-key"><?php echo JText::_('VRCORDERNUMBER'); ?></span> 
					<span class="vrc-order-details-info-val"><?php echo $ord['id']; ?></span>
				</div>
				<div class="vrc-order-details-info-inner">
					<span class="vrc-order-details-info-key"><?php echo JText::_('VRCCONFIRMATIONNUMBER'); ?></span> 
		...
		<div class="vrc-order-details-summary">
			<div class="vrc-order-details-summary-entry">
				<div class="vrc-order-details-summary-key">
					<span><?php echo JText::_('VRDAL'); ?></span>
				</div>
			...
			<div class="vrc-order-details-summary-entry">
				<div class="vrc-order-details-summary-key">
					<span><?php echo JText::_('VRAL'); ?></span>
				</div>
				<div class="vrc-order-details-summary-val">
...
<div class="vrc-order-details-costs-wrap">
	<div class="vrc-order-details-costs-inner">
		...
		if (isset($locfeewith) && !empty($locfeewith)) {
			?>
		<div class="vrc-order-details-costs-row">
			<span class="vrc-order-details-costs-name"><?php echo JText::_('VRLOCFEETOPAY'); ?></span> 
			<span class="vrc-order-details-costs-price">
				<span class="vrc_currency"><?php echo $currencysymb; ?></span> 
		...
		if ($usedcoupon == true) {
			?>
		<div class="vrc-order-details-costs-row">
			<span class="vrc-order-details-costs-name"><?php echo JText::_('VRCCOUPON').' '.$expcoupon[2]; ?></span>
			<span class="vrc-order-details-costs-price">
		...
		<div class="vrc-order-details-costs-row vrc-order-details-costs-row-total">
			<span class="vrc-order-details-costs-name"><?php echo JText::_('VRTOTAL'); ?></span> 
			<span class="vrc-order-details-costs-price">
				<span class="vrc_currency vrc_keepcost"><?php echo $currencysymb; ?></span> 
	...
	if ($ord['totpaid'] > 0 && !($ord['totpaid'] > $ord['order_total'])) {
		...
		<div class="vrc-order-details-costs-row vrc-order-details-costs-row-total vrc-order-details-costs-row-totalpaid">
			<span class="vrc-order-details-costs-name"><?php echo JText::_('VRCAMOUNTPAID'); ?></span> 
			<span class="vrc-order-details-costs-price">
				<span class="vrc_currency vrc_keepcost"><?php echo $currencysymb; ?></span> 
	...
	if ($ord['status'] == 'confirmed' && is_array($payment) && VikRentCar::multiplePayments() && $ord['order_total'] > 0 && $payable) {
		...
		<div class="vrc-order-details-costs-row vrc-order-details-costs-row-total vrc-order-details-costs-row-remainingbalance">
			<span class="vrc-order-details-costs-name"><?php echo JText::_('VRCTOTALREMAINING'); ?></span>
			<span class="vrc-order-details-costs-price">
				<span class="vrc_currency vrc_keepcost"><?php echo $currencysymb; ?></span> 
...
if (is_array($payment) && $ord['status'] == 'standby') {
	...
	if (!VikRentCar::payTotal() && $this->nodep != 1) {
		 ...
		 */
		VRCFactory::getPlatform()->getDispatcher()->trigger('onCalculateDepositAmountVikRentCar', [$ord, &$percentdeposit]);
...
if ($ord['status'] == 'confirmed') {
	...
	if ($printer == 1) {
		...
		// Download PDF
		if (is_file(VRC_SITE_PATH . DIRECTORY_SEPARATOR . "resources" . DIRECTORY_SEPARATOR . "pdfs" . DIRECTORY_SEPARATOR . md5($ord['id'] . '_' . $ord['sid']) . '.pdf')) {
			?>
			<p class="vrcdownloadpdf">
				<a href="<?php echo VRC_SITE_URI; ?>resources/pdfs/<?php echo md5($ord['id'] . '_' . $ord['sid']); ?>.pdf" target="_blank"><?php VikRentCarIcons::e('file-pdf'); ?> <?php echo JText::_('VRCDOWNLOADPDF'); ?></a>
			</p>
			<?php
		}
		...
		<div class="vrcordcancbox">
			...
			<a href="javascript: void(0);" id="vrcopencancform" onclick="javascript: vrcOpenCancOrdForm();" class="btn vrc-pref-color-btn"><?php echo JText::_('VRCREQUESTCANCMODOPENTEXT'); ?></a>
				...
				<form action="<?php echo JRoute::_('index.php?option=com_vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" name="vrccanc" method="post" onsubmit="javascript: return vrcValidateCancForm();">
					<div class="vrcordcancform-inner">
						...
						<div class="vrcordcancform-entry-submit">
							<input type="submit" name="sendrequest" value="<?php echo JText::_('VRCREQUESTCANCMODSUBMIT'); ?>" class="btn vrc-pref-color-btn"/>
						</div>
					</div>
