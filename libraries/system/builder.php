...
class VikRentCarBuilder
{
		...
		add_action('load_payment_gateway_vikrentcar', function(&$drivers, $payment)
		{
			...
			switch ($payment)
			{
				...
				case 'paypal_checkout':
					$classname = 'VikRentCarPayPalCheckoutPayment';
					break;
	...
	public static function setupOverridesManager()
	{
		 ...
		 * @since  1.4.0
		 */
		add_filter('vikrentcar_display_view_config_global', function($forms, $view)
		{
			if (!$forms)
			{
				// init forms array
			...
			$forms[__('Page Overrides', 'vikrentcar')] = $html;
		 ...
		 * The message will be displayed only once within the dashboard of VikRentCar.
		 *
		 * @since 1.4.0
		 */
		add_action('vikrentcar_before_display_dashboard', array('VikRentCarInstaller', 'showBreakingChanges'));
	}
}
