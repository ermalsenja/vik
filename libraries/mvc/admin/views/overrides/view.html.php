...
/**
 * VikRentCar Overrides view.
 * @wponly
 *
 * @since 1.6.5
 */
class VikRentCarViewOverrides extends JView
{
	/**
	 * @override
	 * View display method.
		...
		if ($filters['path'])
		{
			...
			if ($supported)
			{
				...
				$filters['code'] = preg_replace_callback(
					"/\*\s*(\/administrator)?\/?components\/com_vikrentcar(.*?)\R/i",
					function($match)
					{
						return '* /wp-content/plugins/vikrentcar/'
							. ($match[1] ? 'admin' : 'site')
							. $match[2] . "\n";
					},
					$filters['code']
	...
	{
		JToolbarHelper::title(__('VikRentCar - Page Overrides', 'vikrentcar'));

		// back to the configuration
		JToolbarHelper::back('JTOOLBAR_BACK', 'index.php?option=com_vikrentcar&view=config');
